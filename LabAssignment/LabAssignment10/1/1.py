...
def interpolateZYXEulerAngles(euler1, euler2, t):         
    angx = lerp(euler1[0],euler2[0],t)
    angy = lerp(euler1[1],euler2[1],t)
    angz = lerp(euler1[2],euler2[2],t)
    return  ZYXEulerToRotMat((angx,angy,angz))
def interpolateZYXEuler(euler1, euler2,t):
    return interpolateZYXEulerAngles(euler1,euler2,t)
...
def main():
    ...
    if not glfw.init():
        return
    window = glfw.create_window(480,480,'2016025923-10-1', None,None)
    if not window:
        glfw.terminate()
        return
    ...
    while not glfw.window_should_close(window):
        glfw.poll_events()
        # screen changes too fast! so modify it!
        # ang = count % 360
        ang = (count % 3240) /9
        render(ang)
        count += 1
